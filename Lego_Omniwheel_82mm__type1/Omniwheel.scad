...
{
    posZ = offsetSphere-2*h1;    
	translate([0,0,posZ]) drawDoublePinHole();
    rotate([180,0,0]) translate([0,0,posZ]) drawDoublePinHole();
    translate([0,0,-posZ-eps]) cylinder(2*posZ+2*eps, r2, r2,$fn=50);
}
...
module drawBase() 
{
	difference()
	{
		...
		{	
			...
			rotate([0,0,0]) translate([0,pitch,0]) drawFullPinHole();
			rotate([0,0,90]) translate([0,pitch,0]) drawFullPinHole();
			rotate([0,0,180]) translate([0,pitch,0]) drawFullPinHole();
			rotate([0,0,270]) translate([0,pitch,0]) drawFullPinHole();
            drawFancyStuff();
		}
	}
}
